...
class RangeOffice extends Model
{
    ...
    public function scopeLutwbod($query) {
        ...
        if($authUser->userType->default_role == Admin::DEFAULT_ROLE_LIST[5]){
            return $query->where(['forest_range_id' => $authUser->forest_range_id, 'user_type_id' => Admin::RO]);
        } elseif($authUser->userType->default_role == Admin::DEFAULT_ROLE_LIST[4]){
            return $query->where(['forest_division_id' => $authUser->forest_division_id, 'user_type_id' => Admin::RO]);
        } elseif($authUser->userType->default_role == Admin::DEFAULT_ROLE_LIST[3]){
            return $query->where(['forest_state_id' => $authUser->forest_state_id, 'user_type_id' => Admin::RO]); // New
        } elseif($authUser->userType->default_role == Admin::DEFAULT_ROLE_LIST[2]){
            return $query->where(['user_type_id' => Admin::RO]);
        } else{
            return $query->where(['user_type_id' => Admin::RO]);
        }
    }
