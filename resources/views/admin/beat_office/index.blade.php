        ...
        <div class="row mb-2">
          ...
          <div class="col-sm-2">
            @can('create', app('App\Models\RangeOffice'))
              <a href="{{ route('admin.beat_office.create') }}" class="btn btn-info form-control btn-add-new">
          ...
          <div class="col-sm-10">
            <h1 class="text-info">
              <i class="fas fa-bookmark"></i> {{ __('admin.menu.beat_office') }}
            </h1>
          </div>
            ...
            <div class="card">
              ...
              <div class="card-body">
                <table id="example1" class="table table-bordered table-hover">
                  ...
                  <tr>
                    <th>#</th>
                    {{-- <th>{{__('admin.beat_office.user_type')}}</th>
                    <th>{{__('admin.beat_office.role')}} </th> --}}
                    <th>{{__('admin.beat_office.forest_state')}} </th>
                    <th>{{__('admin.beat_office.forest_division')}} </th>
                    <th>{{__('admin.beat_office.forest_range')}} </th>
                    <th>{{__('admin.beat_office.forest_beat')}} </th>
                    
                    <th>{{__('admin.beat_office.name')}}</th>
                    {{-- <th>{{__('admin.beat_office.address')}}</th> --}}
                    <th>{{__('admin.beat_office.code')}}</th>
                    <th>{{__('admin.beat_office.email')}}</th>
                    <th>{{__('admin.beat_office.contact')}}</th>
                    
                    <th>{{__('admin.beat_office.created_at')}}</th>
                    <th>{{__('admin.beat_office.status')}}</th>
                    <th>{{__('admin.beat_office.thumb')}}</th>
                    <th>{{__('admin.beat_office.action')}}</th>
                  </tr>
                  </thead>

                  @foreach ($beat_offices as $key => $admin)
                  <tr>
                    ...
                    <td>
                      @can('update', app('App\Models\RangeOffice'))
                        <a href="{{ route('admin.beat_office.edit', $admin->id) }}" class="btn btn-xs btn-primary"><i class="fas fa-edit"></i></a>
                      @endcan
                      
                      @can('delete', app('App\Models\RangeOffice'))
                        <a href="{{ route('admin.beat_office.delete', $admin->id) }}" href1="{{ route('admin.beat_office.delete', [$admin->id,1]) }}"  class="btn btn-xs btn-danger delete"><i class="fas fa-trash-alt"></i></a>
                      @endcan
...
{{-- <script type="text/javascript">
  $(function () {
    var table = $('#example1').DataTable({
        ...
        ajax: "{{ route('admin.beat_office.datatable') }}",
        columns: [
