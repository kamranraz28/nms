          ...
          <div class="col-12">
            <div class="card">
              <div class="card-header">
                <h3 class="card-title"> @foreach ($cats as $key => $cat) 
                {{ __('admin.dashboard.seedlingwise_stock')}} ( {{ EnglishToBanglaDate::dateFormatEnglishToBangla (date('d-m-Y'))}}) {{ __('admin.dashboard.till_stock')}} -  {{$cat['title_'. app()->getLocale()]}}
                ...
                <table id="example1" class="table table-bordered table-hover">
                  <thead>
                    <tr>
                      ...
                      <th scope="col">{{ __('admin.menu.forest_division') }}</th>
                      <th scope="col">{{ __('admin.dashboard.stock_quantity') }}</th>
                    </tr>
                  </thead>
                <tbody>
                 @foreach ($yearly_stock_seedling_forestdivision_wise as $key => $item)
                  <tr>
                    <td>{{(app()->getLocale() == 'en') ? $key+1 : NumberToBanglaWord::engToBn($key+1)}}</td>
                    <td><a href="{{route('admin.dashboard.view-seedling-range',[$id,$item['id'] ] )}}">{{$item['title_'. app()->getLocale()]}}</a></td>
