          ...
          <div class="col-12">
            <div class="card">
              ...
              <div class="card-body">
                ...
                <tbody>
                  @foreach ($yearly_stock_seedlink_wise_datas as $key => $item)
                  <tr>
                    <td>{{(app()->getLocale() == 'en') ? $key+1 : NumberToBanglaWord::engToBn($key+1)}}</td>
                    <td>{{$item['category_'. app()->getLocale()]}}</td>
                    <td>@if ($authUser->userType->default_role <= Admin::DEFAULT_ROLE_LIST[3])<a href="{{route('admin.dashboard.view-seedling',$item['id'] )}}">{{$item['title_'. app()->getLocale()]}}</a>@else {{$item['title_'. app()->getLocale()]}} @endif</td>
